...
if [ "${API_AUTHTYPE}" = "basic" ]; then
    curl -XDELETE -u "${API_KEY}:${API_SECRET}" "${API_ENDPOINT}/ak/api/v3/kubernetes/components/purge"
    ...
    curl -XDELETE -H "Authorization: Bearer ${ACCESS_TOKEN}" "${API_ENDPOINT}/ak/api/v3/kubernetes/components/purge"
fi
