...
This role allows you to quickly build Hyperledger Fabric components for a multi-zone, highly available endorsing organization. A multi-zone, highly available endorsing organization
has a certificate authority with multiple replicas and multiple peers distributed across multiple zones.
...
----------
  ...
  zones (required)
    The list of zones, for example ``dal10``, ``dal12``, and ``dal13``, that the Red Hat OpenShift or Kubernetes cluster has been configured with. The peers created by this role will be distributed across these zones.
...
.. code-block:: yaml+jinja
  ...
  - name: Create components for a multi-zone, highly available endorsing organization
    vars:
      ...
      wait_timeout: 3600
    roles:
      - hyperledger.fabric-ansible-collection.mzha_endorsing_organization
